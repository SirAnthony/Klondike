...
import {FlightKindSelect, FlightStatusSelect, FlightTypeSelect} from '../util/inputs'
import {LocationSelect, OwnerSelect, TimeInput} from '../util/inputs'
...
function FlightRowNew(props: FlightRowNewParams){
    ...
    const [kind, setKind] = React.useState(flight?.kind)
    ...
    const onSubmit = async ()=>check() && (await props.onSubmit({_id: flight?._id,
        type, kind, ts: +date.get(ts), owner, location, status})) && props.onCancel()
    return <RB.Row className='menu-input-row'>
      ...
      <RB.Col>
        <FlightKindSelect value={kind} onChange={setKind} />
      </RB.Col>
      <RB.Col>
