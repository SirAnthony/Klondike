...
export function LocationSelect(props: {value?: Location, onChange: (loc: Location)=>void}){
    ...
    return <RB.Container>
      <RB.Row><RB.Col>
        <PSelect value={location} onChange={locChange} />
      </RB.Col>
      <RB.Col>
...
export function MultiOwnerSelect(props: {value?: ID[],
    ...
    const owners = props.value?.map(o=><RB.Container key={`sel_owner_${o._id}`}>
      {o.name}<RB.Button onClick={()=>removeOwner(o._id)}>X</RB.Button>
    </RB.Container>)
...
export function MultiResourceSelect(props: {value?: ResItem[],
    ...
    const resources = props.value?.map(o=><RB.Container key={`sel_res_${o.kind}`}>
      {L(`res_kind_${o.kind}`)} [{o.value}]<RB.Button onClick={()=>removeRes(o.kind)}>X</RB.Button>
    </RB.Container>)
    return <RB.Row className={props.className}>
      <RB.Col sm={2}>
